...
Der folgende Text soll die Metriken der Tests in JUnit zeigen, welche Art von Abdeckung sie gemäß den Programmklassen, den Funktionen und den Zeilen haben, auf die über den Testprozess zugegriffen wird.\\

In den Tests wurden die Methoden der entsprechenden Klassen aufgerufen und es wurde überprüft, dass die Daten korrekt verarbeitet wurden, damit die Methode funktionieren kann. Auf die gleiche Weise möchte ich überprüfen, ob die Service-Schicht die Daten mit der VERIFY Methode aufruft. Die Methoden wurden auch mit manuellen Tests zur Integration in verschiedenen Schichten getestet.\\

Um eine Annäherung an die Variablen und Funktionen jeder Klasse vorzunehmen, wurde das Intellij Plugin TESTME verwendet, das die Möglichkeit bietet, eine Ansicht der Methoden in jeder Klasse zu erstellen. Anschließend wurden die Tests manuell vollständig reformiert, um sie verwenden zu können das Projekt. Die Einschränkungen der JUnit Tests wurden durch manuelle Tests und automatisierte Tests in Selenium abgedeckt.\\
...
Projektklassen wurden unabhängig getestet. Das für die Tests verwendete Framework war Junit, Version 5. Das Mockito Framework für die automatischen Tests wurde ebenfalls verwendetet. Mock Objekte wurden für die zu testenden Klassen geschrieben und in der Zeit vor der Abgabe wurden so viele Tests wie möglich durchgeführt.

Für die meisten Klassen wurde ein unabhängiger Test durchgeführt, bei dem die Methoden separat getestet wurden. Zusätzlich zu einer breiten Testabdeckung wurden Selenium Tests und manuelle Tests durchgeführt, um die Abdeckung der automatischen Tests zu erweitern und die Mindestanforderungen der Software zu kontrollieren. 
...
Die folgenden Grafiken zeigen die Abdeckung jeder Klasse pro Paket. Die Klassen, die nicht automatisch getestet wurden, wurden für die endgültige Lieferung drastisch geändert, aus diesem Grund wurden die manuellen Tests durchgeführt, um diese Klassen abzudecken.
