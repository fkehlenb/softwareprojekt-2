...
Nun möchten wir die Controller in unserem System betrachten. Diese Module sind für die Logik zuständig, welche von der WebUI ausgelöst wird. Möchte zudem z. B. ein Prozesskettenadministrator eine neue Prozesskettenvorlage erstellen, so darf die WebUI nicht direkt auf die Datenklassen verweise machen, sondern muss diese Instruktion erst einmal an einen Controller weitergeben welcher diese anschließend realisiert. Zudem haben diese Controller auch zugriff auf die Persistence Klassen, welche wir zunächst betrachten werden, um Objekte die von einer Datenklasse durch den Controller erstellt wurden auch in der Datenbank abspeichern zu können.   
\begin{sidewaysfigure} 
\begin{center}
...
Hier wird das DAO-Pattern genutzt, wodurch die Programmlogik sich nicht an technische Details der Datenspeicherung halten muss. Dadurch ist ein flexibelerer Einsatz möglich. 
...
\end{center}
\end{figure}
Dieses Modul ist für die Erstellung und Verwaltung von Nutzern zuständig. Durch User Controller können z. B. neue Nutzer erstellt werden, oder die Daten von bestehenden Usern verändert werden. Dafür greift Controller auf die Datenklasse User zu, um diese Nutzer zu erstellen und zu verwalten. Um diese anschließend in der Datenbank abspeichern zu können, greift User Controller auf die UserDAO zu, um die Benutzer in der Datenbank zu speichern und zu verwalten. In der Datenklasse von User wird zudem auch die eingestellte Sprache des Nutzers eingespeichert und nach Wunsch geändert.

\newpage

\subsubsection{Prozess Ketten Vorlage Modul}
\begin{figure}[h!]
\begin{center}
...
\end{center}
\end{figure}
Dieses Modul ist für die Erstellung von Prozesskettenvorlagen zuständig. Um eine neue Prozesskettenvorlage erstellen zu können, brauchen wir allerdings erst einmal Prozessschrittvorlagen, aus welchen diese bestehen soll. Um nun eine Prozesskettenvorlage erstellen zu können, muss der Controller Zugriff auf die Datenklasse von Prozesskettenvorlage haben. Nachdem eine neue Prozesskettenvorlage erstellt wurde, möchte man diese allerdings in der Datenbank speichern. Hierfür hat Controller auch Zugriff auf die DAO Klasse von Prozesskettenvorlage. Das Gleiche gilt auch, wenn man eine Prozesskettenvorlage bearbeiten oder löschen möchte. 
\newpage
...
Nun kommen wir zu unserem Auftragsmodul. Aufträge sollen instanziierte Prozessketten sein. Daher benötigen wir, um einen Auftrag erstellen zu können, eine Prozesskettenvorlage, aus welcher dieser erstellt wird. Ein Auftrag muss allerdings auch immer einen AuftragsLog haben, in welchem alles mögliche Wichtige geloggt wird. Ein Auftrag besteht allerdings aus Prozessschritten, welche er daher auch hat.
...
\end{center}
\end{figure}
Nun möchte man ja vielleicht Prozessschritte erstellen. Ein Prozessschritt benötigt einen Prozessschrittszustandsautomaten und einen Prozessschrittlog. Um nun Prozessschritte erstellen zu können, muss unser Prozessschrittcontroller Zugriff auf die Prozessschrittdatenklasse haben, und um diese anschließend in der Datenbank verwalten zu können brauch er auch Zugriff auf ProzessschrittDAO.
\newpage 
...
Nun kommen wir zum Modul Probe. Die Proben in unserem System müssen erstellt und verwaltet werden können, wodurch der Probencontroller Zugriff auf die Probendatenklasse haben muss. Außerdem muss man die Proben in der Datenbank auch verwalten können, wodurch der Probencontroller Zugriff auf ProbeDAO haben muss.
